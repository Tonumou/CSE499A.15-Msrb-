...
│
├── data/
│   ├── xView2 Challenge Dataset - train and test/ # Raw satellite imagery and JSON labels
│   ├── processed_images/        # Standardized JPEGs (Max edge 1280px) ready for VLM
│   ├── processed_labels/        # Paired JSON metadata used for prompt context
│   ├── processed_tracker.txt    # State tracking file to manage API generation batching
│   └── dataset/                 # Expected output dir for final train_dataset.jsonl
│
├── support/                     # Core data engineering and API generation scripts
│   ├── image_standardizer.py    # Resizes/standardizes raw PNGs to safe-VRAM JPEGs
│   └── generate_ground_truth.py # Multimodal Gemini API training text generation
│
├── src/                         # (Planned Phase 2) Core model training & evaluation
│   ├── training/                # Unsloth QLoRA fine-tuning Jupyter notebooks
│   └── evaluation/              # ROUGE-L, BERTScore, and RAR scoring scripts
│
├── app/                         # (Planned Phase 3) Hugging Face Spaces deployment
│   ├── app.py
│   └── requirements.txt
│
├── Proposal Report.pdf          # Academic framework and research validation
├── Proposal Presentation.pptx   # Defense and presentation slide deck
├── requirements.txt             # Python dependencies (GenerativeAI, PIL, etc.)
├── .gitignore                   # Safety ignores for heavy imagery and cache files
└── README.md